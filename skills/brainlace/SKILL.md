...
6. Use `brainlace_related` when the user gives a loose idea and LIN needs likely context.
7. Use `brainlace_plan_note_update` when deciding whether a thought belongs in an existing note or a new note.
8. Use `brainlace_create_note` for new durable notes; keep `wire_index=true` unless intentionally creating an unlinked scratch note.
9. Use `brainlace_append_note` for small additions to existing notes.
10. Use `brainlace_patch_note` for targeted replacements and link repairs; inspect the returned diff.
11. Use `brainlace_move_note` for note moves/renames so inbound wikilinks and destination `INDEX.md` wiring can be handled together.
12. Use `brainlace_check_links` after structural changes or when notes feel messy.
